...
namespace qtnion{
	...
	struct quaternion {
		 ...
		 * @param rhs 加数
		 */
		inline quaternion operator+(const quaternion& rhs) const noexcept(noexcept(operator+<T>))
		 ...
		 * @param rhs 乗数
		 */
		quaternion operator*(const quaternion& rhs) const  noexcept(noexcept(operator+<T>) && noexcept(operator-<T>) && noexcept(operator*<T>)){
			quaternion ret;
		...
		inline bool operator==(const quaternion& rhs) const noexcept(noexcept(operator==<T>)) 
			{ return (one==rhs.one&&i==rhs.i&&j==rhs.j&&k==rhs.k);}
	};
	/**
	 * @brief 四元数に対する共役
	...
	*/
	template <typename T>
	inline quaternion<T> conjugate(quaternion<T> val) noexcept(noexcept(operator-<T>))
	 ...
	 */
	template <typename T>
	inline T squ_norm(quaternion<T> val) noexcept(noexcept(operator+<T>) && noexcept(operator*<T>))
